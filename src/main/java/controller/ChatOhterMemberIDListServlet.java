...
public class ChatOhterMemberIDListServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServerException, IOException{
        ...
        if(nickname == null || nickname.equals("")) {
        	...
        	try { // 채팅 내역 있는 아이디 리스트 반환
        		response.getWriter().write(getOtherMemberNumList(nickname));
        	} catch(Exception e) {
        		e.printStackTrace();
        	}
        }
    ...
    public String getOtherMemberNumList(String nickname) {
    	// 채팅 내역 있는 사용자 no 리스트
    	ChatDao dao = new ChatDao();
    	ArrayList<String> otherNickList = dao.getOtherNickList(nickname);
